...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> curr = child.put(key, rid);
        if (!curr.isPresent()) {
            sync();
            return Optional.empty();
        } else {
            DataBox currkey = curr.get().getFirst();
            long currChild = curr.get().getSecond();
            ...
            if (keys.size() > 2 * order) {
                ...
                List<DataBox> rightkeys = keys.subList(order + 1, limit);
                List<Long> leftchildren = children.subList(0, order + 1);
                List<Long> rightchildren = children.subList(order + 1, limit + 1);
                InnerNode newInner = new InnerNode(metadata, bufferManager, rightkeys, rightchildren, treeContext);
                newInner.sync();
                this.keys = leftkeys;
                this.children = leftchildren;
                sync();
                long pageNum = newInner.getPage().getPageNum();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (data.hasNext() && keys.size() <= 2 * order) {
            ...
            if (childpair.isPresent()) {
                keys.add(childpair.get().getFirst());
                children.add(childpair.get().getSecond());
            }
        }
        if (2 * order <= keys.size()) {
            ...
            InnerNode leftnode = new InnerNode(metadata, bufferManager, rightkeys, rightchildren, treeContext);
            leftnode.sync();
            sync();
            return Optional.of(new Pair<DataBox, Long>(splitkey, leftnode.getPage().getPageNum()));
        } else {
            sync();
            return Optional.empty();
        }
