...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> OpPair = root.put(key, rid);
        if (OpPair.isPresent()) {
            Pair<DataBox, Long> pair = OpPair.get();
            List<DataBox> keys = new ArrayList<>();
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        if (fillFactor <= 0 || fillFactor > 1) {
            throw new BPlusTreeException("Bad fill factor Argument");
        }
        if (root.getLeftmostLeaf()!= root || root.getLeftmostLeaf().scanAll().hasNext()) {
            throw new BPlusTreeException("Tree is not empty at the time of bulkload");
        }
        while (data.hasNext()) {
            ...
            if (pair.isPresent()) {
                ...
                rightkeys.add(pair.get().getFirst());
                rightchildren.add(root.getPage().getPageNum());
                rightchildren.add(pair.get().getSecond());
                InnerNode newroot = new InnerNode(metadata, bufferManager, rightkeys, rightchildren, lockContext);
                updateRoot(newroot);
            }
        }
    }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public boolean hasNext() {
            ...
            Optional<LeafNode> rightsib = currleaf.getRightSibling();
            while(rightsib.isPresent()) {
                currleaf = rightsib.get();
                iterRid = currleaf.scanAll();
                if (iterRid.hasNext()) {
                    return true;
                } else {
                    rightsib =  currleaf.getRightSibling();
                }
            }
            return false;
        }
