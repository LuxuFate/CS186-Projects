...
public class Database implements AutoCloseable {
    ...
    private class TransactionContextImpl extends TransactionContext {
        ...
        public void close() {
            try {
                // TODO(proj4_part2)
                List<Lock> allLocks = lockManager.getLocks(getTransaction());
                Collections.reverse(allLocks);
                for (Lock allLock : allLocks) {
                    LockContext.fromResourceName(lockManager, allLock.name).release(getTransaction());
                }
            } catch (Exception e) {
