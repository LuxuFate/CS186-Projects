...
public class LockManager {
    ...
    private class ResourceEntry {
        ...
        public void grantOrUpdateLock(Lock lock) {
            // TODO(proj4_part1): implement
            for (Lock selectedLock : locks) {
                if (selectedLock.transactionNum.equals(lock.transactionNum)) {
                    selectedLock.lockType = lock.lockType;
                    return;
                }
            }
            locks.add(lock);
            List<Lock> list = transactionLocks.getOrDefault(lock.transactionNum, new ArrayList<>());
            list.add(lock);
    ...
    public void acquireAndRelease(TransactionContext transaction, ResourceName name,
        ...
        synchronized (this) {
            ...
            if (resourceEntry.getTransactionLockType(transaction.getTransNum()).equals(lockType)) {
                throw new DuplicateLockRequestException("Lock is held by transaction");
            }
            ...
            if (!resourceEntry.checkCompatible(lockType, transaction.getTransNum())) {
                ...
                for (ResourceName resourceName : releaseNames) {
                    LockType locktype = getResourceEntry(resourceName).getTransactionLockType(transaction.getTransNum());
                    locks.add(new Lock(name, locktype, transaction.getTransNum()));
                }
                LockRequest lockRequest = new LockRequest(transaction, lock, locks);
                ...
                for (ResourceName resourceName : releaseNames) {
                    ...
                    Lock l = new Lock(resourceName, getLockType(transaction, resourceName), transaction.getTransNum());
                    getResourceEntry(resourceName).releaseLock(l);
                }
            }
        }
        if (shouldBlock) {
    ...
    public void acquire(TransactionContext transaction, ResourceName name,
        ...
        synchronized (this) {
            ...
            Lock lock = new Lock(name, lockType, transaction.getTransNum());
            boolean empty = resourceEntry.waitingQueue.isEmpty();
