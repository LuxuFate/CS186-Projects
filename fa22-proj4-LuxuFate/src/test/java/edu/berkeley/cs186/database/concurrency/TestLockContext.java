...
public class TestLockContext {
    ...
    @Test
    public void testEffectiveLock() {
        TransactionContext t1 = transactions[1];
        dbLockContext.acquire(t1, LockType.IX);
        tableLockContext.acquire(t1, LockType.NL);
        pageLockContext.acquire(t1, LockType.NL);

        assertEquals(LockType.NL, tableLockContext.getEffectiveLockType(t1));
    }

    @Test
    public void testEffectiveLock1() {
        TransactionContext t1 = transactions[1];
        dbLockContext.acquire(t1, LockType.IX);
        tableLockContext.acquire(t1, LockType.SIX);
        pageLockContext.acquire(t1, LockType.NL);

        assertEquals(LockType.SIX, tableLockContext.getEffectiveLockType(t1));
    }

    @Test
    public void testEffectiveLock2() {
        TransactionContext t1 = transactions[1];
