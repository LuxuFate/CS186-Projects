...
public class ARIESRecoveryManager implements RecoveryManager {
    ...
    public long commit(long transNum) {
        ...
        CommitTransactionLogRecord record = new CommitTransactionLogRecord(transNum, entry.lastLSN);
    ...
    public long abort(long transNum) {
        // TODO(proj5): implement
        TransactionTableEntry entry = transactionTable.get(transNum);
        AbortTransactionLogRecord record = new AbortTransactionLogRecord(transNum, entry.lastLSN);
    ...
    public long end(long transNum) {
        ...
        long endRecordLSN = logManager.appendToLog(new EndTransactionLogRecord(transNum, entry.lastLSN));
    ...
    private void rollbackToLSN(long transNum, long LSN) {
        ...
        while (currentLSN > LSN) {
            ...
            currentLSN = logManager.fetchLogRecord(record.getPrevLSN().get()).getLSN();
        }
    }
    ...
    public synchronized void checkpoint() {
        ...
        for (Map.Entry<Long, TransactionTableEntry> entry : transactionTable.entrySet()) {
            ...
            chkptTxnTable.put(entry.getKey(), new Pair<>(transactionTableEntry.transaction.getStatus(), transactionTableEntry.lastLSN));
        }
    ...
    void restartAnalysis() {
        ...
        while (iter.hasNext()) {
            ...
            if (transNumOpt.isPresent()) {
                Transaction transaction = newTransaction.apply(transNumOpt.get());
                if (!transactionTable.containsKey(transNumOpt.get())) {
                    startTransaction(transaction);
                }
                transactionTable.get(transNumOpt.get()).lastLSN = logLSN;
            }
            ...
            switch (logType) {
                ...
                case END_TRANSACTION:
                    Transaction tran = transactionTable.get(logRecord.getTransNum().get()).transaction;
                    tran.cleanup();
            ...
            if (logType.equals(LogType.END_CHECKPOINT)) {
                ...
                for (Map.Entry<Long, Pair<Transaction.Status, Long>> entry : logRecord.getTransactionTable().entrySet()) {
                    long tableTransNum = entry.getKey();
                    if (endedTransactions.contains(tableTransNum)) {
                        continue;
                    }
                    if (!transactionTable.containsKey(tableTransNum)) {
                        startTransaction(newTransaction.apply(tableTransNum));
                    }
                    TransactionTableEntry tableEntry = transactionTable.get(tableTransNum);
                    if (entry.getValue().getSecond() != null &&
                            entry.getValue().getSecond() >= tableEntry.lastLSN) {
                    ...
                    Transaction.Status tableStatus = transaction.getStatus();
                    if (chkptStatus.equals(Transaction.Status.COMPLETE)) {
                        transaction.setStatus(chkptStatus);
                    } else {
                        if (tableStatus.equals(Transaction.Status.RUNNING)) {
        ...
        for (Map.Entry<Long, TransactionTableEntry> entry : transactionTable.entrySet()) {
            ...
            switch (transaction.getStatus()) {
                ...
                case RUNNING:
                    transaction.setStatus(Transaction.Status.RECOVERY_ABORTING);
                    entry.getValue().lastLSN = logManager.appendToLog(new AbortTransactionLogRecord(tableTransNum, entry.getValue().lastLSN));
                    break;
    ...
    void restartRedo() {
        ...
        while (iter.hasNext()) {
            ...
            if (logRecord.isRedoable()) {
                List<LogType> partition = Arrays.asList(
                        LogType.ALLOC_PART, LogType.FREE_PART, LogType.UNDO_ALLOC_PART, LogType.UNDO_FREE_PART);
                if (partition.contains(logType)) {
                    logRecord.redo(this, diskSpaceManager, bufferManager);
                }
                List<LogType> alloc = Arrays.asList(
                        LogType.ALLOC_PAGE, LogType.UNDO_FREE_PAGE);
                if (alloc.contains(logType)) {
                    logRecord.redo(this, diskSpaceManager, bufferManager);
                }
                List<LogType> modify = Arrays.asList(
                        LogType.UPDATE_PAGE, LogType.UNDO_UPDATE_PAGE, LogType.FREE_PAGE, LogType.UNDO_ALLOC_PAGE);
                if (modify.contains(logType)) {
    ...
    void restartUndo() {
        ...
        PriorityQueue<Pair<Long, LogRecord>> abortingTransactions = new PriorityQueue<>(new PairFirstReverseComparator<>());
        for (Map.Entry<Long, TransactionTableEntry> entry : transactionTable.entrySet()) {
            if (entry.getValue().transaction.getStatus().equals(Transaction.Status.RECOVERY_ABORTING)) {
                abortingTransactions.add(new Pair<>(entry.getValue().lastLSN, logManager.fetchLogRecord(entry.getValue().lastLSN)));
            }
        }
        while (!abortingTransactions.isEmpty()) {
            Pair<Long, LogRecord> pair = abortingTransactions.poll();
            ...
            abortingTransactions.add(new Pair<>(newLSN, logManager.fetchLogRecord(newLSN)));
        }
    }
