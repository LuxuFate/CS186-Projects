...
public class SortOperator extends QueryOperator {
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        while (!priorityQueue.isEmpty()) {
            ...
            Iterator<Record> iter = iterators.get(pair.getSecond());
            if (iter.hasNext()) {
                priorityQueue.add(new Pair<Record, Integer>(iter.next(), pair.getSecond()));
            }
    ...
    public List<Run> mergePass(List<Run> runs) {
        // TODO(proj3_part1): implement
        List<Run> merged =  new ArrayList<Run>();
        int bucket = numBuffers - 1;
        for (int i = 0; i < runs.size(); i += bucket) {
            Run r = mergeSortedRuns(runs.subList(i, i + bucket));
            merged.add(r);
        }
        return merged;
    }
    ...
    public Run sort() {
        ...
        // TODO(proj3_part1): implement
        List<Run> sortedRun = new ArrayList<Run>();
        while (sourceIterator.hasNext()) {
            BacktrackingIterator<Record> iter = getBlockIterator(sourceIterator, getSource().getSchema(), numBuffers);
            sortedRun.add(sortRun(iter));
        }
