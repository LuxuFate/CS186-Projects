...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int minIndexCost = sequentialCost;
        List<Integer> indices = getEligibleIndexColumns(table);
        int except = -1;
        QueryOperator indexOp = null;
        for (int i : indices) {
            SelectPredicate sp = selectPredicates.get(i);
            indexOp = new IndexScanOperator(this.transaction, table, sp.column, sp.operator, sp.value);
            int indexCost = indexOp.estimateIOCost();
            if (indexCost < minIndexCost) {
                except = i;
                minIndexCost = indexCost;
            }
        }
        if (minIndexCost < sequentialCost) {
            return addEligibleSelections(indexOp, except);
        } else {
        ...
        for (Set<String> tables : prevMap.keySet()) {
            for (JoinPredicate jp : this.joinPredicates) {
                ...
                if (tables.contains(leftTable) && !tables.contains(rightTable)) {
                    set.add(rightTable);
                    minOp = minCostJoinType(prevMap.get(tables), pass1Map.get(set), leftColumn, rightColumn);
                } else if (!tables.contains(leftTable) && tables.contains(rightTable)) {
                    set.add(leftTable);
                    minOp = minCostJoinType(prevMap.get(tables), pass1Map.get(set), rightColumn, leftColumn);
                } else {
                    continue;
                }
                set.addAll(tables);
                if (!result.containsKey(set) || minOp.estimateIOCost() < result.get(set).estimateIOCost()) {
                    result.put(set, minOp);
                }
            }
        }
        return result;
    ...
    public Iterator<Record> execute() {
        ...
        Map<Set<String>, QueryOperator> passimap = pass1map;
